    ...
    internal class AuthTokenService : IAuthTokenService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            if (!string.IsNullOrEmpty(setting.UserName) && !string.IsNullOrEmpty(setting.Password))
                return await GetAccessToken(setting.Scopes, httpClient, setting.AuthorisationUrl, setting.ClientId,
                    setting.UserName, setting.Password, cancellationToken);

            return await GetAccessToken(setting.Scopes, httpClient, setting.AuthorisationUrl, setting.ClientId, setting.ClientSecret, cancellationToken);
        }
